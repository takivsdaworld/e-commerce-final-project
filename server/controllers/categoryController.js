...
const removeCategory = asyncHandler(async (req, res) => {
  try {
    ...
    res.json({ message: "Category deleted successfully", deletedCategory: removed });
  } catch (error) {
    console.error(error);
...
const listCategory = asyncHandler(async (req, res) => {
  try {
    ...
    console.error(error);
    return res.status(400).json({ error: "Could not fetch categories" });
  }
});
...
const readCategory = asyncHandler(async(req, res) => {
    try {
        ...
        console.error(error);
        return res.status(400).json({ error: "Could not fetch category" });
